...
void transpose_columns(code_t *H) {
    for (index_t i = 0; i < INDEX; ++i) {
        transpose(H->rows[i], H->columns[i], BLOCK_WEIGHT, H->blocklength);
    }
}

void transpose_rows(code_t *H) {
    for (index_t i = 0; i < INDEX; ++i) {
        transpose(H->columns[i], H->rows[i], BLOCK_WEIGHT, H->blocklength);
    }
}
...
void compute_codeword(cw_t codeword, code_t *H, msg_t message) {
    ...
    for (index_t k = 0; k < INDEX; ++k) {
        multiply_xor_mod2(codeword[k], H->columns[INDEX - 1 - k], message,
                          BLOCK_WEIGHT, H->blocklength);
    }
#else
    index_t sizeavx = ROUND_UP(H->blocklength * 8 * sizeof(bit_t), 256 * 16) / 8;
    for (index_t k = 0; k < INDEX; ++k) {
        multiply_xor_mod2_avx2(codeword[k], H->rows[INDEX - 1 - k], message,
                               BLOCK_WEIGHT, sizeavx);
    }
#endif
}
...
void compute_syndrome(syndrome_t *syndrome, code_t *H, e_t *e_dense) {
    ...
    for (index_t j = 0; j < H->blocklength; ++j) {
...
void compute_counters(counters_t counters, bit_t *syndrome, code_t *H) {
    memcpy(syndrome + H->blocklength, syndrome, H->blocklength * sizeof(bit_t));
    for (index_t i = 0; i < INDEX; ++i) {
#ifndef AVX
        memset(counters[i], 0, H->blocklength * sizeof(bit_t));
...
void error_sparse_to_dense(e_t *e_dense, const sparse_t e_sparse,
    ...
    for (; k < weight; ++k) {
        index_t j = e_sparse[k] - blocklength;
        e_dense->vec[1][j] ^= 1;
    }
    for (index_t i = 0; i < INDEX; ++i) {
        memcpy(e_dense->vec[i] + blocklength, e_dense->vec[i],
               blocklength * sizeof(bit_t));
    }

    e_dense->weight = weight;
    e_dense->initial_weight = weight;
}
